...
Pose2D    utm_link_origin;   // utm_link의 원점 pose2D
...
int initialize_waypoint(void)  // waypoint를 초기화 하기 위한 함수
{
...
{
    // 축의 변환 utm 좌표를 90도로 시계방향으로 회전하면 map 좌표가 됨

    set_rotation_matrix_axis(-utm_origin.yaw); // 축의 변환 utm 좌표를 90도로 시계방향으로 회전하면 map 좌표가 됨
