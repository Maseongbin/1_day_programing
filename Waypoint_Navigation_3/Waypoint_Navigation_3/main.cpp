...
Line_Equation waypoint_line_equation_utm[15];        // utm 좌표 기준 waypoint의 line equation
Line_Equation waypoint_line_equation_map[15];        // map 좌표 기준 waypoint의 line equation
...
double Rotation_matrix_point[2][2];    // 점을     변환하는 회전 행렬
double Rotation_matrix_axis[2][2];     // 좌표축을 변환하는 회전 행렬

int initialize_waypoint(void)   // waypoint 초기화 함수
{
...
{
    double distance = 0;   // y =a*x + b -> a*x + b - y =0
...
int main(void)
{
    ...
    int current_waypoint_id = 3;  // 현재 목표점 waypoint ID
    ...
    cross_track_error_utm = calculate_distance_point_line_equation_utm(test, current_waypoint_id);
    cross_track_error_map = calculate_distance_point_line_equation_map(test, current_waypoint_id);
